// just get the word that doesn't appear twice!
input = `xsddbi (61)
nqtowev (11)
xwohr (82)
flejt (36)
...
wqokqz (50)
xufneyr (153)`.replace(/\([0-9]+\)/g, '').replace(/->/g, '').replace(/,/g, '').replace(/\t/g, '').replace(/\n/g, ' ').split(' ').sort().filter(s => s != "");

for(let i = 0; i < input.length; i += 2) {
    if(input[i] != input[i + 1]) {
        console.log(input[i - 1] + " | " + input[i] + " | " + input[i + 1]);
        break;
    }
}