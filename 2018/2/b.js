let input = `3458 3471 163 1299 170 4200 2425 167 3636 4001 4162 115 2859 130 4075 4269
...
input.forEach(string => {
    ...
    for(let i = 0; i < numbers.length; i++) {
        for(let j = i + 1; j < numbers.length; j++) {
            let a_div_b = parseInt(numbers[i]) / parseInt(numbers[j]);
            let b_div_a = parseInt(numbers[j]) / parseInt(numbers[i]);
            // console.log(`${numbers[i]} / ${numbers[j]} = ${a_div_b}`);
            // console.log(`${numbers[j]} / ${numbers[i]} = ${b_div_a}`);
            if(Number.isInteger(a_div_b)) {
                checksum += a_div_b;
                // console.log(`Found ${a_div_b}`);
            }
            if(Number.isInteger(b_div_a)) {
                checksum += b_div_a;
                // console.log(`Found ${b_div_a}`);
            }
        }
    }
